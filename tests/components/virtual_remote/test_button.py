"""Tests for Virtual Remote command buttons."""

from unittest.mock import AsyncMock, MagicMock, Mock, patch

from homeassistant.components.virtual_remote.button import (
    VirtualRemoteButton,
    VirtualRemoteButtonEntityDescription,
    button_unique_id,
...
async def test_button_press_sends_command(
    ...
    with patch(
        "homeassistant.components.virtual_remote.button.async_send_infrared_command",
        AsyncMock(),
    ) as mock_send:
...
def test_cleanup_stale_button_entities_removes_only_matching_stale_entries(
    ...
    with (
        patch(
            "homeassistant.components.virtual_remote.button.er.async_get",
            return_value=registry,
        ),
        patch(
            "homeassistant.components.virtual_remote.button.er.async_entries_for_config_entry",
            return_value=[
                stale_button,
                expected_button,
                other_domain,
...
async def test_async_setup_entry_skips_button_command_without_payload(
    ...
    with patch(
        "homeassistant.components.virtual_remote.button.normalize_command_objects",

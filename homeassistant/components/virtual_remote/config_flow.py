...
class VirtualRemoteConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_reconfigure(
        ...
        """Handle reconfiguration of the virtual remote."""
        entry = self._get_reconfigure_entry()
        remote = virtual_remote_from_config_entry_data({**entry.data, **entry.options})
        infrared_entities = available_infrared_entities(self.hass)
    ...
    async def async_step_reconfigure_codeset(
        ...
        if remote is None:
            entry = self._get_reconfigure_entry()
            remote = virtual_remote_from_config_entry_data(
